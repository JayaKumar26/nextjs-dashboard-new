...
const createInvoice = async (formData: FormData) => {
  ...
  try {
   ...
   VALUES (${customerId}, ${amountInCents}, ${status}, ${date})`;
    ...
    console.error(error);
    return { message: "Database Error: Failed to Create Invoice", status: 500 };
  }
...
const updateInvoice = async (id: string, formData: FormData) => {
  ...
  try {
    ...
    console.error(error);
    return { message: "Database Error: Failed to Update Invoice", status: 500 };
  }
  revalidatePath("/dashboard/invoices");
  redirect("/dashboard/invoices");
};
...
const deleteInvoice = async (id: string) => {
  ...
  try {
    ...
    console.error(error);
    return { message: "Database Error: Failed to Delete Invoice", status: 500 };
  }
  revalidatePath("/dashboard/invoices");
};
